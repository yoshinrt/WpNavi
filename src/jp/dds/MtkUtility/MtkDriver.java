...
public class MtkDriver implements Runnable{
	...
	byte [] LogBuf		= null;
	volatile boolean	bFailSector;
	volatile boolean	bReadLogCompleted;
	volatile boolean	bSetInterval;
	...
	final int SECTOR_SIZE			= 64 * 1024;	// 64KB
	final int HEADER_SIZE			= 512;
	final int DYNAMIC_PATTERN_SIZE	= 16;
	...
	public int Open( String BTName ){
		...
		try{
			...
			if( bDebug ) Log.d( "MtkUtility", "MtkDriver::Open:timeout" );
		}catch( IOException e ){
	...
	int GetRecordSize(){
		...
		while( iLogSize < 0 ) try{ Thread.sleep( 100 ); }catch( Exception e ){};
		return iLogSize;
	}
	...
	int GetFailedSector(){
		...
		while( bFailSector ) try{ Thread.sleep( 100 ); }catch( Exception e ){};
		return 0;
	}
	...
	int SetInterval( int ms ){
		...
		while( bSetInterval ) try{ Thread.sleep( 100 ); }catch( Exception e ){};
		return 0;
	}
	...
	int GetInterval(){
		...
		iInterval = -1;
		SendCmd( "PMTK400" );
		while( iInterval < 0 ) try{ Thread.sleep( 100 ); }catch( Exception e ){};
		return iInterval;
	}

	// ログ取得
	int GetLog( ProgressDialog Dialog ){
		bReadLogCompleted = false;

		if( OutStream == null ) return -1;

		if(
			( iLogSize < 0 && GetRecordSize() < 0 ) ||
			( FailSector == null && GetFailedSector() < 0 )
		){
			return -1;
		}
		...
		while( bReadLogCompleted == false ) try{
			if( WaitDialog != null ){
				WaitDialog.setProgress( iStartAddr );
			}
			Thread.sleep( 100 );
		}catch( Exception e ){};
		...
		if( "$PMTK".equals( new String( Buf, iStart, 5 ))){
			...
			switch( iCmd ){
			  case 0x00010182:	// return cmd status
				if( ParseHex( Buf ) == 0x7 ) bReadLogCompleted = true;
				break;
	...
	final int FMT_UTC		= ( 1 << 0 );
	final int FMT_VALID		= ( 1 << 1 );
	final int FMT_LATITUDE	= ( 1 << 2 );
	final int FMT_LONGITUDE	= ( 1 << 3 );
	final int FMT_HEIGHT	= ( 1 << 4 );
	final int FMT_SPEED		= ( 1 << 5 );
	final int FMT_TRACK		= ( 1 << 6 );
	final int FMT_DSTA		= ( 1 << 7 );
	final int FMT_DAGE		= ( 1 << 8 );
	final int FMT_PDOP		= ( 1 << 9 );
	final int FMT_HDOP		= ( 1 << 10 );
	final int FMT_VDOP		= ( 1 << 11 );
	final int FMT_NSAT		= ( 1 << 12 );
	final int FMT_SID		= ( 1 << 13 );
	final int FMT_ELE		= ( 1 << 14 );
	final int FMT_AZI		= ( 1 << 15 );
	final int FMT_SNR		= ( 1 << 16 );
	final int FMT_RCR		= ( 1 << 17 );
	final int FMT_MS		= ( 1 << 18 );

	final int SIZE_UTC		= 4;
	final int SIZE_VALID	= 2;
	final int SIZE_LATITUDE	= 8;
	final int SIZE_LONGITUDE= 8;
	final int SIZE_HEIGHT	= 4;
	final int SIZE_SPEED	= 4;
	final int SIZE_TRACK	= 4;
	final int SIZE_DSTA		= 2;
	final int SIZE_DAGE		= 4;
	final int SIZE_PDOP		= 2;
	final int SIZE_HDOP		= 2;
	final int SIZE_VDOP		= 2;
	final int SIZE_NSAT		= 2;
	final int SIZE_SID		= 4;
	final int SIZE_ELE		= 2;
	final int SIZE_AZI		= 2;
	final int SIZE_SNR		= 2;
	final int SIZE_RCR		= 2;
	final int SIZE_MS		= 2;
